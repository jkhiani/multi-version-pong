function singlePlayerHardInstructions(){
	disable();
	...
	function changeBg(){
		canvas.style.background = 'url("singlePlayerEasyInstructions.jpg")';
		x++;
...
function singlePlayerHard(){
	...
	var compPaddleMoveSpd = 4.5;
	var compPaddlePosition = (canvas.width-paddleWidth1)/2;
	...
	function draw(){
		...
		if (compPaddlePosition+(compPaddleWidth/2)<ballX){
			compPaddlePosition += compPaddleMoveSpd;
		}
		else if (compPaddlePosition+(compPaddleWidth/2)>ballX){
			compPaddlePosition -= compPaddleMoveSpd;
		}
